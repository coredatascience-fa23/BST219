...
df[df$infant_mortality < 10,]

### Multiple combinations
filter(df, 
       infant_mortality < 10 | life_expectancy > 80)
...
df[order(df$infant_mortality),] ### this is kind of ugly

### use desc() for decreasing
arrange(df, desc(infant_mortality))
df[order(df$infant_mortality, decreasing = T),] ### even more ugly

### sort by continent, then region
arrange(df, continent, region)
df[order(df$continent, df$region),] ### even more ugly


### First sort by contient [A-Z] then in descending order of IMR
df4 <- arrange(df, continent, desc(infant_mortality))
```
...
df_complex[order(df_complex$country, df_complex$year),]
...
select(filter(gapminder, country == 'Algeria'), gdp, year)
