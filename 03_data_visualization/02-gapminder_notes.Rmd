...
gapminder %>% 
  ...
  xlab("Fertility (avg number of children per woman)") +
  ylab("Life expectancy (years)") + 
  scale_color_manual(values = c('pink', 'purple')) ### for custom colors
...
gapminder %>% 
  filter(year %in% years & continent %in% continents) %>%
  ggplot(aes(x = fertility, y = life_expectancy, color = as.character(year))) + 
  geom_point() + 
...
```{r, warning=FALSE}
### Compute average fertility by year
df_avg <- 
  gapminder %>% 
  group_by(year, continent) %>% 
  summarize('mean_fertility' = mean(fertility)) %>% 
  ungroup()
...
gapminder %>% 
  ...
  geom_line(data = df_avg, aes(x = year, y = mean_fertility, color = continent),lwd = 2) + 
  # geom_point() +
  xlab("Year") +
  ylab("Fertility (avg number of children per woman)")
```
